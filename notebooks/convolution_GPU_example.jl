...
# ╔═╡ 02814ffd-991c-4672-8791-168564e5a869
using CUDA, TestImages, IndexFunArrays, PlutoUI, ImageShow, FFTW

# ╔═╡ a2714150-f737-4ddc-a2b7-c9ada01a58e7
using DifferentiationInterface, Optim, Optim

# ╔═╡ 23d7995b-7cbd-4e42-97da-1e503db13920
import Enzyme, Zygote, ReverseDiff

# ╔═╡ 2615f780-089a-41d6-a043-505a21d441f7
TableOfContents()

# ╔═╡ 7d6ed1ef-3271-4cbd-8f5e-e13a7b22a4e1
...
# ╔═╡ 0ce337ba-6c40-4008-9cb6-79e3533f72d7
togoc(x) = use_CUDA[] ? CuArray(x) : x

# ╔═╡ b01b8419-45d4-42f2-aee8-99316ccfd9ab
md"# Background

In this example we want to demonstrate an inverse optimization problem.
Given an blurry measurement $Y$ we seek to recover the image $X$ which has been convolved with $h$:
...
# ╔═╡ 453eae77-2781-49d8-81db-da3b47bb0a59
md"# Load an image and create blurring kernel h"

# ╔═╡ 03291d99-3c4d-4280-b3ff-4b273475efc1
X = togoc(Float32.(testimage("resolution_test_512")));

# ╔═╡ 780a71fb-be1f-44f5-992c-ef788ee24cf0
h = togoc(normal(Float32, size(X), sigma=3));

# ╔═╡ 0aac6cb2-92e5-4514-ae2c-49404e4fbef9
md"# Create a forward model"

# ╔═╡ 334de576-c89c-4fda-8a1d-a7fb0ef87fd6
"""
	conv(x, h)

Implement a FFT based convolution operation
"""
function conv(x, h)
	return real(ifft(fft(x) .* fft(ifftshift(h))))
end;

# ╔═╡ 9794931f-1bcf-45e8-a5b4-7b251d55986a
...
# ╔═╡ 409965c7-65dd-419b-83d6-ec3d2d6f52f2
md"# Define loss function and gradient"

# ╔═╡ 88cb7289-e76d-4a99-bd16-dbb48cc3023e
loss(x) = sum(abs2, Y .- conv(x, h))

# ╔═╡ 40819db4-0b56-4a4c-993e-309d19eb0dae
gradient!(G, x) = DifferentiationInterface.gradient!(loss, G, AutoZygote(), x)

# ╔═╡ f3a9bbc3-ad1b-47f4-9a9f-5ef4ca83d602
md"# Do the optimization"
...
manifest_format = "2.0"
project_hash = "af23174bb9e289d726733ce893782d605586260d"
...
deps = ["ADTypes", "Compat", "DocStringExtensions", "FillArrays", "LinearAlgebra", "PackageExtensionCompat", "SparseArrays"]
git-tree-sha1 = "bfc5660e0642d0f02e4707f8ead1f065e87fe165"
...
git-tree-sha1 = "5051d46f8795cd1adb16016f6b7145d42e98297f"
...
git-tree-sha1 = "117141562896ca38b1a13bc515dfd2728bd86e55"
